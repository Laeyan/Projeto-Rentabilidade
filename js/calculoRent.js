...
function montali(dado, nomeClass) {
	...
	function calculaAnos(Nome, Aporte, Rent, RentAnual, Anos, divAnosjs, divRenAnosjs, divAnosFinais, divTituloEntrada){
		...
		while(i < Anos){
			if(i < Anos && v == 0){
				divTituloEntrada.appendChild(montah2("Olá "+ Nome + ", com R$" + Aporte + " de aportes mensais depositados todos os meses em um investimento que possua rentabilidade de "+ RentAnual +"% ao ano, você pode conseguir rentabilidades de: ", "tituloEntrada"));
				ValorAcul.push(0);
			}
			if(m==0 && i==0){
				PorcentagemAno.push(0);
			...
			ValorAcul.push(ValorAcul[Position] + Aporte + Porcentagem[Position]).toFixed(2);
			...
			if(m==11){
				...
				divRenAnosjs.appendChild(montaparagrafo("No "+ i + " ano a rentabilidade ganha foi de: R$" + (ValorAcul[Position].toFixed(2)-ValorSemRen[Position-1]-PorcentagemAno[quantAnos]).toFixed(2),"PorcentagemAnual"));
				quantAnos++;
				divRenAnosjs.appendChild(montaparagrafo("No "+ i + " ano o valor acumulado é de: R$" + (ValorAcul[Position].toFixed(2)-ValorSemRen[Position-1]).toFixed(2),"PorcentagemTotal"));
				PorcentagemAno.push((ValorAcul[Position].toFixed(2)-ValorSemRen[Position-1]).toFixed(2));
		...
		if(!(i < Anos)){
			...
			divAnosFinais.appendChild(montah3("O valor total ganho com o investimento comparado a somente poupar é de: R$"+ (ValorAcul[Position].toFixed(2)-ValorSemRen[Position-1]).toFixed(2),"tituloFinalGanho"));
